    ...
    return (
      <div className="fixed bottom-4 right-4 bg-white rounded-full shadow-lg p-3 cursor-pointer hover:bg-gray-50">
        <div className="relative">
          ...
          {(messages.some(m => !m.read && m.senderRole !== currentUserRole) || hasNewMessage) && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          messages.map((message, index) => (
            <div key={index} className={`flex ${message.senderRole === currentUserRole ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-lg px-4 py-2 ${message.senderRole === currentUserRole ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                <p className="break-words">{message.content}</p>
                <p className={`text-xs mt-1 ${message.senderRole === currentUserRole ? 'text-indigo-100' : 'text-gray-500'}`}>{formatTimestamp(message.timestamp)}</p>
              </div>
            </div>
          ))
        )}
